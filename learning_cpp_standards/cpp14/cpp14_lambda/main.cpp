...
namespace lambda_ns {
    ...
    void test_lambda_14() {
        ...
        // 注意: `std::vector nums{...}` 这种类模板实参推导(CTAD)是 C++17 才有的,
        // C++14 下必须显式写出模板参数。
...
namespace lambda_capture_forward {
    ...
    auto create_lambda(T &&value) {
        ...
        return [capture = std::forward<T>(value)]() mutable {
            ...
            if (std::is_same<T, std::string>::value) {
                ...
                std::cout << "string&&";
            } else if (std::is_same<T, Pointer>::value) {
                std::cout << "Pointer";
            } else if (std::is_same<T, Pointer &>::value) {
                std::cout << "Pointer&";
            } else if (std::is_same<T, Pointer &&>::value) {
    ...
    void test_pointer() {
        ...
        auto lambda4 = [](Pointer &value) {
            std::cout << "T的类型萃取: ";
            if (std::is_same<decltype(value), Pointer>::value) {
                std::cout << "Pointer";
            } else if (std::is_same<decltype(value), Pointer &>::value) {
                std::cout << "Pointer&";
            } else if (std::is_same<decltype(value), Pointer &&>::value) {
                std::cout << "Pointer&&";
            }
