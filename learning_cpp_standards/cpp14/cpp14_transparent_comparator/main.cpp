...
namespace transparent_comparator_ns {
    ...
    void test() {
        ...
        // 注意: std::plus{} 这种类模板实参推导(CTAD)是 C++17 引入的,
        // C++14 下必须显式写出 std::plus<> (等价于 std::plus<void>)。
    ...
    void my_test() {
        ...
        // 注意: std::string_view 与 ""sv 字面量是 C++17 引入的,
        // C++14 下用 const char* / std::string 代替。
        ...
        // auto it6 = m2.find("cherry"); // 报错，因为 const char* 不能隐式转换为 string 类型进行比较
        ...
        // 透明比较器支持异构查找: 这里用 std::find + 显式比较器演示
        auto it = std::find_if(v1.begin(), v1.end(), [](const std::string &s) { return s == "hello"; });
        if (it != v1.end()) {
    ...
    void my_test3() {
        ...
        // 注意: `[](const auto &vec)` 这种泛型 lambda 是 C++14 就支持的,
        // 但下面还用到 `logical_and{}` 之类的 CTAD(C++17), 已一并改为
        // 显式模板实参写法。
...
namespace logical_ns {
    ...
    void my_test() {
        ...
        // 注意: `logical_and{}` 的 CTAD 是 C++17 的, C++14 需写 `logical_and<>{}`。
