...
// 	1.std::basic_syncbuf：是std::basic_streambuf的包装器(wrapper)，它将输出累积在其自己的内部缓冲区中，并在销毁时以及明确请求时将其全部内容自动传输到包装的缓冲区，以便它们显示为连续的字符序列。此类
// 模板主要用于创建与多线程程序无缝协作的自定义流类。如果多个线程试图同时写入同一个流，std::basic_syncbuf将确保每个线程的输出都以正确的顺序写入流，而不会出现任何交错。
