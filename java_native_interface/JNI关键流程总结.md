...
| 文件 | 作用 |
| --- | --- |
| `java_call_cpp/pom.xml` | Maven 构建配置，设置 `Main-Class`，并在打包时复制 `hello.dll` 到 `target` |
| `java_call_cpp/src/main/java/com/App.java` | Java 程序入口，调用 `Hello.helloFromCPP()` |
| `java_call_cpp/src/main/java/com/Hello.java` | 声明 native 方法，并按 JAR 目录优先加载 DLL |
...
`java_call_cpp` 是一个 Maven JAR 工程，入口类是 `com.App`。参照 `java_native_access/java_call_cpp` 后，当前工程结构改为：

- `maven-jar-plugin` 在 JAR 清单中写入 `Main-Class: com.App`，因此可以直接使用 `java -jar`。
- `maven-resources-plugin` 在 `prepare-package` 阶段把 `java_call_cpp/hello.dll` 复制到 `target`，让 DLL 与 JAR 同级。
- `Hello` 加载 DLL 时优先查找类或 JAR 所在目录下的 `hello.dll`；如果该位置不存在，则回退到 `user.dir/hello.dll`，兼容原来的 classpath 运行方式。
...
java_call_cpp/
    ...
    hello.dll
    target/
        java_call_cpp-1.0-SNAPSHOT.jar
        hello.dll
```
...
flowchart LR
    ...
    B --> C["Hello 静态初始化"]
    C --> D["System.load(JAR 同级或 user.dir 下的 hello.dll)"]
    D --> E["JVM 加载 DLL"]
...
- 使用 `java -jar target/java_call_cpp-1.0-SNAPSHOT.jar` 时，DLL 应位于 JAR 同级目录，即 `target/hello.dll`。
- 使用 `java -cp target/classes com.App` 时，如果 `target/classes/hello.dll` 不存在，会回退加载当前工作目录下的 `hello.dll`。
...
MinGW/CMake 生成的是 `libhello.dll`，但当前 Java 代码加载的文件名是 `hello.dll`，所以需要先复制并改名到 `java_call_cpp` 目录。
...
随后执行 `mvn package` 时，Maven 会再把 `java_call_cpp/hello.dll` 复制到 `java_call_cpp/target/hello.dll`，使它与可执行 JAR 同级。

`libhello.dll.a` 是供其他原生程序链接 DLL 时使用的导入库，Java 运行本例不需要它。项目的 Git 忽略规则也不会提交 DLL，因此新环境需要重新构建并执行复制步骤。

## 10. 打包并使用 java -jar 测试

在 `java_call_cpp` 目录执行：

```bash
mvn package
java -jar ./target/java_call_cpp-1.0-SNAPSHOT.jar
```

`mvn package` 会完成 Java 编译、生成 `target/java_call_cpp-1.0-SNAPSHOT.jar`，并把 `hello.dll` 复制到 `target` 目录。

也可以保留 classpath 方式做快速测试：
...
```text
Hello World!
C:\source\learning_cpp_vscode\java_native_interface\java_call_cpp\target\hello.dll
im from cpp
```
...
| `Can't load ... hello.dll` | `java -jar` 时检查 `target/hello.dll` 是否存在；classpath 运行时检查当前目录是否有 `hello.dll` |
