...
int test_ranges_classes() {
    ...
    std::cout << "data:" << *sv1.data() << ";begin:" << *sv1.begin() << ";size:" << sv1.size() << ";distance:" << std::distance(sv1.begin(), sv1.end())
              << std::endl; // data:3.14159;begin:3.14159;size:1;distance:1
    ...
    std::cout << "empty:" << rv.empty() << ",size:" << rv.size() << ",begin:" << *rv.begin() << ",end:" << *(rv.end() - 1) << ",data:" << rv.data()
              << std::endl; // empty:0,size:5,begin:M,end:y,data:Messy_Test
    for (const auto c : rv)
        std::cout << c; // Messy
    std::cout << std::endl;
