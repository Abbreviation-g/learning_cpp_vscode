...
namespace shared_timed_mutex_ns {
    ...
    public:
        ...
        std::string read() const {
            ...
            std::stringstream ss;
            ss << std::format("[read: {} 线程] 正在读数据", std::this_thread::get_id()) << std::endl;
            std::cout << ss.str();
        ...
        void write(const std::string &data) {
            ...
            std::cout << std::format("[write: {} 线程] 正在写数据", std::this_thread::get_id()) << std::endl;

            std::this_thread::sleep_for(std::chrono::milliseconds(1000));
            m_data = data;

            std::cout << std::format("[write: {} 线程] ，数据更新完毕, 新数据:{}", std::this_thread::get_id(), m_data) << std::endl;
        }
    ...
    void test() {
        ...
        std::cout << std::format("最终的数据: {}", data.read()) << std::endl;
    }
} // namespace shared_timed_mutex_ns
int main() {
