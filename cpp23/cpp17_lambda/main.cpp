...
namespace {
    ...
    public:
        void f() {
            ...
            auto l4 = [i, *this] {
                use(i, x);
            }; // C++17: 拷贝 *this, 包括 x 的副本
            i = 4;
            x = 4;
            l4(); // use(3, 2) — i 和 x 都按值拷贝
        }

    private:
        int x{ 0 };

        void use(int i, int x) const {
            std::cout << "i = " << i << ", x = " << x << "\n";
        }
    };
    ...
    public:
        ...
        auto getLambdaWithCopyOfThis() const {
            return [*this] {
                return std::string("name(copy): ") + name;
            };
        }

        auto getLambdaWithThisRef() const {
            return [this] {
                return std::string("name(ref):  ") + name;
            };
        }
...
constexpr auto addOne2(int n) {
    return [n] {
        return n + 1;
    };
}
constexpr auto addOne3 = [](int n) {
    return n + 1;
};
